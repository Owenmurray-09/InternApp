...
export default function EmployerDashboard() {
  ...
  const loadJobs = async () => {
    try {
      ...
      console.log('Error message:', companyError?.message);

      if (companyError) {
        // Handle "no data found" error (company doesn't exist)
        ...
        // Handle permissions/RLS errors (406, PGRST301, etc.)
        if (
          companyError.code === 'PGRST301' ||
          companyError.message?.includes('406') ||
          companyError.message?.includes('Not Acceptable') ||
          companyError.status === 406
        ) {
          console.log('Database permissions issue, assuming company exists and continuing');
          console.log('Setting empty jobs array and continuing...');
          setJobs([]);
          setLoading(false);
          return;
        }

        // For other errors, log and continue gracefully
        console.error('Company query error (continuing anyway):', companyError);
        setJobs([]);
        setLoading(false);
        return;
      }
